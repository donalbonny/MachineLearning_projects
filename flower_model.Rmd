...
Domain-specific genes targeted by floral quartets that determine different floral organ identities, i.e., sepals, petals, stamens, and carpels, as colored in the diagram of floral organs.



Dataset includes the expression changes between 2 different domains  to identify potential regulators affect domain specificity.

Based on domain-specific mRNA translatome data ( see reference), they identified 1013 organ-specific genes that were highly expressed either in AP1-specific (sepal), AG-specific (carpel), AP1/AP3-common (petal), or AP3/AG-common (stamen) domains




Use this data to build the model to predict whether the new instances (new genes) will be controlled by more than 1 floral regulator. In other words, which cluster the gene belongs to. 
...
```{r}
#Look at the cluster districution
library(caret)
percentage <- prop.table(table(data$Cluster)) * 100
cbind(freq= table(data$Cluster), percentage = percentage)
```
So there are 5 different clusters: AG-Specific, AP1 specific, AP1-AP3, AP3 specific and AP3-AG that can control the instances (target genes)

There are 6 features can be used to build the model: AP1-Stage 4, AP1-Stage6, 
...
t-SNE (t-distributed stochastic neighbor embedding) plot showing the transcription factor (TF)-binding profiles of organ-specific genes
...
# classification and dicision tree 
set.seed(2018)
